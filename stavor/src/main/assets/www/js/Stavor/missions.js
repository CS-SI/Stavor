...
function onDeleteMissionButtonClicked(){
	if(global_missions.active == -1){
		Dialog.showDialog("str_dialog_click_mission_first_title", "str_dialog_click_mission_first_message", function(){});
	}else{
		if(global_missions.active == global_missions.selected){
			Dialog.showDialog("str_dialog_cannot_delete_mission_title", "str_dialog_cannot_delete_mission_message", function(){});
		}else{
			db.transaction(function (tx) {
				tx.executeSql('SELECT name FROM missions WHERE id = '+global_missions.active+';', [], function (tx, results) {	
					Dialog.showConfirmDialog("str_dialog_confirm_delete_mission_title",["str_dialog_confirm_delete_mission_message_1",results.rows.item(0).name,"str_dialog_confirm_delete_mission_message_2"],function(){
						db.transaction(function (tx) {
		...
		if(global_missions.active != global_missions.selected){
			db.transaction(function (tx) {
				tx.executeSql('SELECT * FROM missions WHERE id = '+global_missions.active+';', [], function (tx, results) {	
					Dialog.showConfirmDialog("str_dialog_confirm_select_mission_title",["str_dialog_confirm_select_mission_message_1",results.rows.item(0).name,"str_dialog_confirm_select_mission_message_2"], function(){
...
function onCopyMissionButtonClicked(){
	if(global_missions.active == -1){
		...
		db.transaction(function (tx) {
			tx.executeSql('SELECT * FROM missions WHERE id = '+global_missions.active+';', [], function (tx, results) {	
				Dialog.showConfirmDialog("str_dialog_confirm_copy_mission_title",["str_dialog_confirm_copy_mission_message_1",results.rows.item(0).name,"str_dialog_confirm_copy_mission_message_2"], function(){
					db.transaction(function (tx) {
...
function resetMissionsDb(){
	Dialog.showConfirmDialog("str_dialog_reset_missions_db_title","str_dialog_reset_missions_db_message",function(){
		db.transaction(function (tx) {
