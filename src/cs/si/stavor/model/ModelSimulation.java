...
public class ModelSimulation {
    ...
    public synchronized void updateHUD(){
    	if(selectedBrowser.equals(Browsers.Attitude)){
    		...
    		if(panel_roll != null)
    			panel_roll.setText("Rol: "+String.format("%.1f", (180*info.roll/Math.PI))+"�");
    		if(panel_pitch != null)
    			panel_pitch.setText("Pitch: "+String.format("%.1f", (180*info.pitch/Math.PI))+"�");
    		if(panel_yaw != null)
    			panel_yaw.setText("Yaw: "+String.format("%.1f", (180*info.yaw/Math.PI))+"�");
    	}else if(selectedBrowser.equals(Browsers.Map)){
    		if(panel_time != null)
    			panel_time.setText(info.time.replace("T", "  "));
    		if(panel_progress != null)
